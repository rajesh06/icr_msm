...
severity_eda <- function(df){
  ...
  lapply(X = unique(df$lag_to_close), FUN = function(closing_lag){
    #closing_lag <- 1
...
* There are certain claims that do not result in any additional payment. We see included the 45$^\circ$ line on the plots to highlight these claims. 
...
* We also note that it's possible that the likelihood of no further payments may be a function of the payments to date. That is, we may observe there are more 'off-the-line' points in the central region of the x-axis. However, we are not certain as to whether that's due to there simply being more observation in that region or a true predictive phenomenon.
