Using the synthetic data that was generated in Section 1 we will discuss how to organize the data in a way that allows us to measure state transitions.
...
For each claim, the synthetic data includes 8 features which are detailed below:
...
The synthetic data then simulates a settlement delay for each individual claim up to a maximum of 11 years. This simulation occurs in fields "Open00" through "Open11" with the value of "1" indicating the claim is open at the corresponding lag and the value of "0" indicating the claim is closed at the corresponding lag. Payments for each lag are also simulated in the fields "Pay00" through "Pay11".  

To determine the transition matrices we will only need the claim number and the simulated fields ("Open00" through "Open11" and "Pay00 through Pay11"). As discussed in Section 2.1, if you were to further refine the states you could include other fields such as age of the claimant or body part injured and model the settlement lags separately based on those characteristics. 

Now that we have a better understanding of the data we are ready to manipulate the data into a utilizable format which will enable us to create the Markov Transition Matrices. The objective is to transform the data into a database with 12 instances for each claim number (maturities 00 through 11) with indicators for whether the claim is open or closed and whether there is value at each maturity.
...
transitions <- unique(markov_data$mat_ymyp1) %>% sort() #define trnasitions
transitions <- transitions[1:length(transitions) - 1]

markov_states <- c('oz', 'onz', 'cz', 'cnz') #definte states

rm(paid_data, status_data) #cleanup

```
The "markov_data" dataframe is now ready to be used to create our Markov Transition Matrices. 

Finally, we use the "mtms" function contained in the `R` package accompanying this paper to create a list of Markov Transition Matrices to be used in our simulation.
