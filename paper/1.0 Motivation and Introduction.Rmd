...
In this paper, we will propose an approach that utilizes granular frequency and severity information to model ultimate values for claims. This paper should not be read as a research paper as that is not the intent. We do not present this paper as a treatise on all possible approaches to modeling individual claims. Rather, our goal is to present a practical approach (i.e., a recipe) accompanied by an `R` package that readers can use to execute the recipe. 

Consistent with that goal our intent is to provide a fully reproducible "worked example" with this paper. That is, we strive to present a _practical approach_ rather a theoretical approach. We also present and open-source the related code to encourage further development of this model. We also try to keep the "words" in the report to a minimum. This is intentional it will require you to the read the code which will improve your understanding of the model. We use `R` for this analysis; we assume that out code-based languages would also be able to efficiently execute this code. We think it would be difficult - but not impossible - to execute this model in Excel.

To that end, we generate synthetic data using the [Simulation Engine](https://people.math.ethz.ch/~wmario/simulation.html) developed by Gabrielli and Wüthrich, M.V.[^1] (convert to bibliography for final) 

This data does not include incurred values. As a result, we present a paid development model. Application to incurred data would be analogous to the application presented in this paper. The primary difference is that incurred information would allow for the consideration of additional variables such as case reserves and paid to incurred ratios.
